...
const DeleteModalKapal = ({tipe = "datang", isOpen, setIsOpen, rowId, onDeleteSuccess}) => {
  const {token} = useAuth();
  if (!isOpen) return null;

  const hapusRow = async () => {
    try{

      const response = await fetch(`http://localhost:3001/api/kapal/${tipe}/` + rowId, {
        method: 'DELETE',
        headers: {
...
export default DeleteModalKapal