...
const DeleteModalKru = ({tipe = "sign_on", isOpen, setIsOpen, rowId, onDeleteSuccess}) => {
  const {token} = useAuth();
  ...
  const hapusRow = async () => {
    try{
      ...
      const response = await fetch(`http://localhost:3001/api/kru/${tipe}/` + rowId, {
        method: 'DELETE',
        headers: {
