...
const EditModal = ({ tipe="datang", isOpen, setIsOpen, rowId, onEditSuccess }) => {
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if(tipe === "berangkat") setDate(new Date(data.jadwal_keberangkatan).toISOString().substring(0, 10));
          else setDate(new Date(data.jadwal_kedatangan).toISOString().substring(0, 10));
        } else {
  ...
  // }, [jadwal_keberangkatan, jadwal_kedatangan, tipe])
  ...
  const editRow = async (e) => {
    ...
    formDataa.append("dokument", file);
    formDataa.append("nama_agen_kapal", nama_agen_kapal);
    formDataa.append("perusahaan_agen_kapal", perusahaan_agen_kapal);
    formDataa.append("imo_number", imo_number);
    formDataa.append("nama_kapal", nama_kapal);
    formDataa.append("kebangsaan_kapal", kebangsaan_kapal);
    formDataa.append("data_cru_indonesia", data_cru_indonesia);
    formDataa.append("data_cru_asing", data_cru_asing);
    formDataa.append("pelabuhan_asal", pelabuhan_asal);
    formDataa.append("pelabuhan_tujuan", pelabuhan_tujuan);
    formDataa.append("service_location", service_location);
    if(tipe === "datang"){
      formDataa.append("jadwal_kedatangan", jadwal_kedatangan);
      formDataa.append("tujuan_kedatangan", tujuan_kedatangan);
    }else{
      formDataa.append("tujuan_keberangkatan", tujuan_keberangkatan);
      formDataa.append("jadwal_keberangkatan", jadwal_keberangkatan);
    }
          ...
          <div className="p-4 md:p-5 ">
            ...
            >
              <div className="grid gap-[20px]">
                ...
                <div>
                  <div className="pl-[17px] font-bold">
                    Jadwal {tipe === "datang" ? "Kedatangan" : "Keberangkatan"} Kapal
                  </div>
                  <div className="pt-[11px] relative">
                    <input
                      type="date"
                      ...
                      onChange={(e) => {tipe === "datang" ? setJadwalKedatangan(e.target.value) : setJadwalKeberangkatan(e.target.value)}}
                      className="pl-[17px] pr-[20px] bg-[#83B3CA] placeholder-[#5C5C68] w-[594px] h-[45px] rounded-[10px] appearance-none"
                    />
                  </div>
                </div>
                <div>
                  <div className="pl-[17px] font-bold">Tujuan {tipe === "datang" ? "Kedatangan" : "Keberangkatan"}</div>
                  <div className="pt-[11px] relative">
                    <select
                      className="pl-[17px] bg-[#83B3CA] placeholder-[#5C5C68] w-[594px] h-[45px] rounded-[10px] appearance-none"
                      value={tujuan_kedatangan}
                      onChange={(e) => {tipe === "datang" ? setTujuanKedatangan(e.target.value) : setTujuanKeberangkatan(e.target.value)}}
                    >
